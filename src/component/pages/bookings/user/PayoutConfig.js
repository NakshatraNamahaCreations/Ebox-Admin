  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setGST(profile.company_gst || "");
        setCIN(profile.company_cin || "");
        setPAN(profile.company_pan || "");
        setSACCODE(profile.company_saccode || "");
        setRazorpayDeduction(profile.razorpay_percentage || 0);
        setTDSDeduction(profile.tds_percentage || 0);
        setTNC(profile.terms_and_conditions || "");
      }
    } catch (error) {
